...
export function AuthProvider({ children }: { children: React.ReactNode }) {
 ...
 async function checkSession() {
  try {
   ...
   const isOffline = msg.includes("network") || msg.includes("timed out") || msg.includes("fetch");
   
   if (isOffline) {
    try {
     const cached = await AsyncStorage.getItem("cachedUser");
     const user = cached ? JSON.parse(cached) : { _id: "offline", username: "Offline User" };
     dispatch({ type: "SET_USER", payload: user });
    } catch {
     dispatch({ type: "SET_USER", payload: { _id: "offline", username: "Offline User" } });
    }
    return;
   }
